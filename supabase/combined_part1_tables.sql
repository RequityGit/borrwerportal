...
RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'loan_events is append-only. UPDATE and DELETE are forbidden. To correct an error, insert a new event with type ''adjustment''.';
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
DO $$ BEGIN CREATE TYPE billing_cycle_status AS ENUM ('draft', 'reconciled', 'submitted', 'complete'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE billing_line_item_status AS ENUM ('pending', 'paid', 'partial', 'delinquent', 'waived'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE interest_method_type AS ENUM ('dutch', 'non_dutch'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE servicing_loan_status AS ENUM ('pending', 'active', 'delinquent', 'in_default', 'paid_off', 'charged_off'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE servicing_payment_type AS ENUM ('regular', 'payoff', 'partial', 'returned'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE late_fee_type AS ENUM ('flat', 'percentage'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE payment_structure_type AS ENUM ('interest_only', 'principal_and_interest'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
UPDATE servicing_loans SET interest_method = CASE WHEN dutch_interest = true THEN 'dutch'::interest_method_type ELSE 'non_dutch'::interest_method_type END WHERE interest_method IS NULL OR interest_method != (CASE WHEN dutch_interest = true THEN 'dutch'::interest_method_type ELSE 'non_dutch'::interest_method_type END);

-- Sync payment_structure
UPDATE servicing_loans SET payment_structure = CASE WHEN payment_type = 'Principal & Interest' THEN 'principal_and_interest'::payment_structure_type ELSE 'interest_only'::payment_structure_type END WHERE payment_structure IS NULL;

-- Sync servicing_status
UPDATE servicing_loans SET servicing_status = CASE WHEN loan_status = 'Active' THEN 'active'::servicing_loan_status WHEN loan_status = 'Paid Off' THEN 'paid_off'::servicing_loan_status WHEN loan_status = 'In Default' THEN 'in_default'::servicing_loan_status ELSE 'active'::servicing_loan_status END WHERE servicing_status IS NULL;
...
-- Trigger to keep interest_method in sync
CREATE OR REPLACE FUNCTION sync_interest_method() RETURNS TRIGGER AS $$ BEGIN IF NEW.dutch_interest IS DISTINCT FROM OLD.dutch_interest THEN NEW.interest_method := CASE WHEN NEW.dutch_interest THEN 'dutch'::interest_method_type ELSE 'non_dutch'::interest_method_type END; END IF; RETURN NEW; END; $$ LANGUAGE plpgsql;
...
DO $$ BEGIN CREATE TYPE delinquency_bucket AS ENUM ('current', '1-30', '31-60', '61-90', '90+'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE ach_account_type AS ENUM ('checking', 'savings'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
WHERE sl.routing_number IS NOT NULL AND sl.account_number IS NOT NULL AND sl.routing_number != '' AND sl.account_number != ''
ON CONFLICT DO NOTHING;

-- ===================== RLS POLICIES =====================

DO $$ BEGIN CREATE POLICY loan_events_admin_all ON loan_events FOR ALL USING (is_admin()) WITH CHECK (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY billing_cycles_admin_all ON billing_cycles FOR ALL USING (is_admin()) WITH CHECK (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY billing_items_admin_all ON billing_line_items FOR ALL USING (is_admin()) WITH CHECK (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY delinquency_admin_all ON delinquency_records FOR ALL USING (is_admin()) WITH CHECK (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY borrower_ach_admin_all ON borrower_ach_info FOR ALL USING (is_admin()) WITH CHECK (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
