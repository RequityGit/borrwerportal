...
// ── Property Details ──
// Primary: layout-driven sections from the Page Layout editor (page_layout_sections + page_layout_fields).
// Fallback: field_configurations via useUwFieldConfigs (grouped by sectionGroup).
...
function PropertyDetailsContent({
  ...
  // Layout-driven sections for the property tab
  const layoutSections = useMemo(() => {
    if (layout.loading) return [];
    return layout.fieldSections.filter((s) => s.tab_key === "property");
  }, [layout.loading, layout.fieldSections]);

  const useLayout = layoutSections.length > 0;

  // Fallback: field_configurations grouped by sectionGroup
  ...
  // Combined field map used for enrichment (accepts any known property field)
  const enrichFieldMap = useMemo(() => {
    ...
    return map;
  }, [fallbackFieldMap, useLayout, layoutSections, layout.fieldsBySectionId, uwFieldMap]);
  ...
  // ── Enrich button (shared across render paths) ──
  const enrichButton = (
    <div className="flex items-center justify-between">
      <div />
      {enriching ? (
  ...
  // ── Primary: layout-driven rendering ──
  if (useLayout) {
    return (
      <div className="space-y-6">
        ...
        {layoutSections.map((section) => {
          ...
          return (
            <div key={section.id} className="rounded-xl border bg-card p-5">
              ...
              <div className="grid grid-cols-12 gap-x-6 gap-y-2.5">
                {layoutFields.map((lf) => {
                  ...
                  const spanClass = lf.column_span === "full" ? "col-span-12" : lf.column_span === "third" ? "col-span-12 sm:col-span-4" : "col-span-12 sm:col-span-6";
                  return (
  ...
  // ── Fallback: field_configurations via useUwFieldConfigs ──
  if (propertyFields.length === 0) {
    if (cardType.property_field_groups.length > 0) {
      const cardTypeFieldMap = new Map(cardType.property_fields.map((f) => [f.key, f]));
      return (
        <div className="space-y-6">
          {enrichButton}
          {cardType.property_field_groups.map((group) => {
            const fields = group.fields
              .map((key) => cardTypeFieldMap.get(key))
